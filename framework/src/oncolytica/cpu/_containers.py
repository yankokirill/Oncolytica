...
from oncolytica.core._types import BaseData, vec3
...
class Grid:
    ...
    def __init__(self, struct_class: Type[BaseData], shape: tuple[int, ...]) -> None:
        ...
        while not it.finished:
            ...
            inst._grid = self
            mi = it.multi_index
            inst._x = mi[0] if len(mi) > 0 else 0
            inst._y = mi[1] if len(mi) > 1 else 0
            inst._z = mi[2] if len(mi) > 2 else 0
    ...
    def sample(self, pos: vec3) -> BaseData:
        """Return the voxel nearest to world-space position *pos*.

        Assumes ``1 grid cell = 1 world-space unit`` (floor mapping).
        Coordinates are clamped to the valid index range.
        """
        ndim = len(self._shape)
        coords = (pos.x, pos.y, pos.z)[:ndim]
        index = tuple(
            int(max(0, min(self._shape[i] - 1, int(coords[i]))))
            for i in range(ndim)
        )
        return self._data[index]
