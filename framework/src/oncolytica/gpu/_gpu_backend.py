...
from oncolytica.gpu._device_manager import DeviceManager, SORT_KERNELS
from oncolytica.gpu._memory import StructSerializer
from oncolytica.gpu._compiler import WGSLCompiler
from oncolytica.gpu._type_system import wgsl_alignment
...
class GpuBackend(ISimulationBackend):
    ...
    def compile(self, sim_instance: Any) -> None:
        ...
        self._spec = sim_instance.spec

        # 1. Initialize memory layouts
        self._ser["tissue"] = StructSerializer("Tissue", self._spec.tissue_class)
        self._ser["chem"] = StructSerializer("Chemistry", self._spec.chemistry_class)
        self._ser["cell"] = StructSerializer("Cell", self._spec.cell_class)
        self._ser["metric"] = StructSerializer("Metrics", self._spec.metrics_class, is_atomic=True)

        self._extract_user_params(sim_instance)

        # 2. Compile Python rules to WGSL
        compiler = WGSLCompiler(self._spec, self.engine, self._user_params, self._ser)
        ...
        self._device_mgr.create_buffers(
            ...
            tissue_grid_dim=tgd, chem_grid_dim=cgd,
    ...
    def get_config(self) -> dict[str, Any]:
        """
        Returns the complete launch configuration needed for the Bevy visualizer.
        Contains shader, struct offsets, uniform layouts, bind groups and execution order.
        """
        ...
        base_uniforms = [
            ("tissue_grid_dim", "vec3<u32>"),
            ("tissue_voxel_size", "f32")
        ]
        ...
        execution_order.append({
            "pass": "sort",
            "kernels": [
                "Kernel_ClearVoxels", "Kernel_CountAndOffset", "Kernel_Scan_L0",
                "Kernel_Scan_L1", "Kernel_Scan_L2", "Kernel_AddBack_L1",
                "Kernel_AddBack_L0", "Kernel_Scatter"
            ],
            "iterations": 1
        })
        ...
        return {
            "compute_shader": self._shader_code,
            "params": params_dict,
            "layout": {
            ...
            "buffers": {} # Будет заполнено в dump_config путями к дампам GPU-памяти
        }

    def dump_config(self, filepath: str = "launch_config.json", export_initial_state: bool = True) -> None:
        """
        Dumps the launch configuration to a JSON file.
        Also exports initial GPU buffers to binary files (`.bin`).
        These binary files act as memory pointers/transfers for Rust Bevy.
        """
        ...
        if export_initial_state:
            ...
            # Передаем пути к бинарникам (указатели на контент буферов)
            config["buffers"] = {
        ...
        print(f"Saved launch configuration and buffers to {filepath}")

    def _extract_user_params(self, sim_instance: Any) -> None:
        for name in sorted(vars(sim_instance)):
            if name.startswith("_"):
                continue
            val = getattr(sim_instance, name)
            if isinstance(val, bool):
                self._user_params[name] = ("i32", int(val))
            elif isinstance(val, float):
                self._user_params[name] = ("f32", val)
            elif isinstance(val, int):
                self._user_params[name] = ("i32", val)
