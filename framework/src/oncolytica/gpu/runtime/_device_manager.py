...
class DeviceManager:
    def __init__(self) -> None:
        ...
        # Комбинации для MockTissue и Chem: ключ = (tissue_flip, chem_flip)
    ...
    def initialize(self) -> None:
        try:
            ...
            raise RuntimeError("Requires wgpu-py. Install it via 'pip install wgpu'.") from exc
        ...
        # Display GPU details to the console
        if settings.SHOW_GPU_INFO:
            info = adapter.info
            print("-" * 50)
            print(f"Active GPU:   {info.get('device', 'Unknown')}")
            print(f"Vendor:       {info.get('vendor', 'Unknown')}")
            print(f"API Backend:  {info.get('backend_type', 'Unknown')}")  # e.g., Vulkan, D3D12, Metal
            print(f"Adapter Type: {info.get('adapter_type', 'Unknown')}")  # DiscreteGPU vs IntegratedGPU
            print("-" * 50)

        required_features = []
        if "subgroups" in adapter.features:
            required_features.append("subgroups")
            required_features.append("shader-f32-atomic")
    ...
    def create_buffers(
        self, total_agents: int, max_agents: int,
        tissue_grid_dim: tuple[int, int, int],
        cell_stride: int, tissue_stride: int,
        chem_stride: int, metrics_stride: int,
        uniform_data: bytes
    ) -> None:
        wgpu = self._wgpu
        device = self.device
